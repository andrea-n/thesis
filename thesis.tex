...
\subsection{Základ grafu pro vizualizaci}
...
```
\end{markdown*}
%Vypočítané hodnoty již určují body v grafu, v tomto případě \mbox{x-ové} souřadnice úrovní pro každý tým.
...
Záznamy událostí ze hry, ať už ve formátu CSV nebo JSON, musí být nejdříve zpracovány do vhodné datové struktury, se kterou bude moci vizualizace pracovat. Některá data musí být v určitém formátu, jako např. čas strávený v jednotlivých úrovních, aby mohl být vypočítán a vykreslen skládaný graf. Některé údaje musí být ze záznamů vypočítány. Datová struktura, se kterou vizualizace pracuje, je objekt s těmito atributy:
\begin{itemize}
  \item \verb|time| – číslo udávající čas od začátku hry v sekundách,
  \item \verb|levels| – pole klíčů jednotlivých úrovní hry,
  \item \verb|levelsTimePlan| – pole čísel udávájí časový odhad pro každou úroveň v sekundách,
  \item \verb|gameDataset| – pole objektů představující týmy, má popsanou strukturu potřebnou pro použití s \verb|d3.stack|, uchovává však některé informace navíc:
	...
	\end{itemize}
  \item \verb|planDataset| – podobné pole objektů jako gameDataset, avšak bez některých prvků, používá se pro vykreslení časového plánu hry.
\end{itemize}
...
odhad, timeline
datová struktura plandataset
\subsection{Vizualizace herních úrovní}
datová struktura gamedataset
aktualizace měřítka, aktualizace plánu - posunutí šrafování
