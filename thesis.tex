...
\begin{itemize}
  \item \verb|time| – číslo udávající čas od začátku hry v sekundách,
...
Dále jsou vytvořena a nastavena měřítka pro obě osy x a y. Doménou x-ového měřítka je celková odhadovaná délka hry, pro y je to množina týmů. Také je vykreslena osa x a hustota bodů na této ose respektuje šířku vizualizace.
...
\begin{figure}[H]
  ...
  \label{fig:visualizationChart}
\end{figure}
Odhadovaný časový limit pro jednotlivé úrovně je vykreslován způsobem popsaným v podkapitole o základu grafu \ref{chartBase}. Použitá datová struktura je stejná, ale místo skutečné délky úrovní je v ní v ní uložen pouze odhad:
\begin{markdown*}{
  fencedCode
}
```Java
...
Před vykreslením je nutné aktualizovat měřítka, protože týmy již mohly překročit celkový odhad a graf by přesáhl stanovenou šířku vizualizace.\par
Po vykreslení reálných dat je třeba aktualizovat plán, protože se mohlo změnit měřítko. Také je třeba posunout šrafované oblasti plánu, které jsou závislé na aktuálním postupu týmu. Všechny šrafované oblasti s výjimkou té, která se týká aktuální úrovně, jsou skryty. Šrafování, které se týká aktuální úrovně, je posunuto na pozici jejího začátku.
%% byly přidány další levely a je jich teď 6 -> ukázka s 6 levely
\subsection{Události}
\subsection{Sloupce s údaji}
jsou v gridu v samostatných sloupcích, aby byly zarovnané s ovládacími prvky nahoře
nejsou součástí SVG grafu, je ale taky svg, musí se počítat y souřadnice
momentálně dva sloupce, tým, slouží jako osa
možné přidat další
