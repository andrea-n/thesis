...
\thesissetup{
    ...
    TeXkeywords   = {KYPO, Capture the flag, CtF, bezpečnostní hry, vizualizace, D3.js, Angular},
    abstract      = {Cílem této práce je vytvoření webové aplikace pro vizualizaci dat získávaných během bezpečnostních her v~prostředí KYPO, která bude sloužit pro sledování průběhu těchto her a~pro jejich analýzu. Práce představuje prostředí KYPO a~dále se zabývá návrhem, implementací a~testováním bezpečnostních her v~tomto prostředí. Popisuje také použité technologie a~implementaci výsledné vizualizace.},
    thanks        = {Ráda bych poděkovala RNDr. Radkovi Ošlejškovi, Ph.D. za vedení této práce, mé konzultantce Bc. Kristíně Zákopčanové za její cenné rady a~v~neposlední řadě mé rodině za trpělivost a~mému příteli za velkou podporu během tvorby této práce.}
}
...
\usepackage[backend=bibtex,style=iso-numeric,sortlocale=cs_CZ]{biblatex}
\bibliography{bibliography} 
\floatsetup[table]{capposition=top}
\begin{document}
\chapter{Úvod}
%které jsou potřebné pro předvídání kybernetických útoků a při budování obrany proti nim.
Bezpečnostní hry jsou prostředkem k~prohlubování a~procvičování znalostí v~oblasti kybernetické bezpečnosti, kdy díky zapojení účastníků do hry jsou jejich znalosti využívány aktivním způsobem. Prostředí Kybernetického polygonu, pro které byla implementována praktická část této práce, je mimo jiné využíváno k~výuce v~oblasti bezpečnosti prostřednictvím těchto her. Za účelem jejich zdokonalování je třeba sledovat a~analyzovat jejich skutečný průběh. Přehled o~průběhu her je ale důležitý také pro vyučující, kteří je vedou, protože tak získají přehled o~aktuálním postupu hráčů a~mohou identifikovat případné problémy.\par
...
K~uvedeným využitím platformy KYPO patří výuka v~oblasti bezpečnosti a~bezpečnostní školení. Vedle obranných cvičení Cyber Defence eXercises (CDX) do této oblasti patří bezpečnostní hry s~názvem Capture the Flag (CtF). \cite{ctfDesign} Jedná se o~metodu aktivního učení, kdy účastníci přímo využívají své znalosti. CtF hry jsou zaměřeny na získávání a~prohlubování znalostí v~oblasti penetračního testování\footnote{Penetrační testování je proces, při kterém se pomocí současných útočných technik odhalují a~využívají zranitelnosti systému, které by jinak mohli použít skuteční útočníci.}. Účastníci jsou postaveni do role útočníka, což by jim mělo také pomoci při předvídání případných útoků. \cite{lessons2015, ctfDesign}\par
...
\begin{itemize}
  \item pouze útok – hry založené na ofenzivních úlohách, kdy je tým v~pozici útočníka na danou infrastrukturu,
...
\subsection{Moduly}
Základní jednotkou Angular aplikací jsou moduly (tzv.~NgModules). Aplikace má vždy jeden kořenový modul, nazvaný podle konvence \verb|AppModule|, který se stará o~spuštění aplikace. Ta pak může být rozdělena do dalších funkčních modulů.\par
...
%Tento dekorátor pomocí předaného objektu s metadaty popisuje daný modul.
%Dekorátoru je předán objekt s metadaty popisujícími daný modul.
%Tento objekt může obsahovat několik důležitých prvků:
\begin{itemize}
  ...
  \item \verb|providers| – seznam služeb, které jsou zaregistrovány do globální kolekce služeb a~jsou dostupné z~celé aplikace,
  \item \verb|bootstrap| – hlavní kořenová komponenta (ta by měla být nastavena jen v~kořenovém modulu).
\end{itemize}
%exports - podmnožina deklarací, které mají být viditelné z ostatních modulů

\subsection{Komponenty}
Komponenta je zodpovědná za správu části uživatelského rozhraní (view). Její logiku obstarává třída označená dekorátorem \verb|@Component| a~s~pohledem interaguje skrze metody a~vlastnosti (properties). Příkladem může být komponenta, která spravuje hlavičku webové stránky pomocí vlastnosti s~menu a~metodou \verb|onClick|, která obsluhuje kliknutí na jednotlivé prvky menu.\par
%JE DOBŘE TEN PŘÍKLAD????
Komponenty mají životní cyklus a~jsou vytvářeny a~odebírány v~závislosti na tom, jak se uživatel pohybuje v~aplikaci. Na jednotlivé části cyklu je možné navázat vlastní akce pomocí tzv.~hooks metod, mezi které patří například: \cite{angulario}
\begin{itemize}
...
%Šablony kombinují HTML kód se značkami Angularu, tzv. direktivami, které mohou upravit DOM stránky ještě před tím, než je zobrazený uživateli.
...
\begin{itemize}
  ...
  %Následující příklad ukazuje, jakým způsobem je komponenta s~názvem UsersListComponent reprezentována v~šabloně.
...
Vázání dat (data binding) je mechanismus pro předávání dat mezi prvky šablony a~částmi komponenty. Pro vazbu dat je v~šabloně možné použít následující čtyři zápisy, které se liší v~obsahu a~směru předávání dat. \cite{angulario}
\begin{itemize}
...
\end{lstlisting}
  ...
  %Nastavuje vlastnost elementu v šabloně na hodnotu vlastnosti komponenty.
  \begin{lstlisting}[showstringspaces=false]
[property] = "value"
\end{lstlisting}
...
%% Je to Dependency injection správně??? 

\section{Papa Parse}
Papa Parse je knihovna pro JavaScript, která je určena pro parsování textu a~souborů ve formátu CSV. Tuto knihovnu je možné integrovat do Angular aplikací pomocí nadstavby ngx-papaparse.\par
%co je ngx-papaparse???????????
...
\begin{itemize}
  ...
  %\item umožňuje také převádět data opačným směrem do CSV.
\end{itemize}
...
V~dalším kroku byla vizualizace integrována do Angular aplikace, byly přidány interaktivní prvky a~také načítání dat skrze API. O~vytvoření Lifereay portletu s~Angular aplikací se již stará skript, který připravil Bc.~David Beran v~rámci projektu \verb|kypo2-portlet-empty-angular|.
...
\end{itemize}
% – např. využité nápovědy, celkově strávený čas apod.,
...
Tento pohled na hru, zobrazený na obrázku~\ref{fig:visualizationChartProgress}, má význam v~jejím průběhu. Zobrazuje postup a~události hry v~čase odpovídajícím skutečnosti. Týmy tedy nejsou z~levé strany zarovnány a~z~grafu je patrné, kdy tým hru začal. Také je zobrazena časová osa vyznačující aktuální okamžik a~čas v~pravém horním rohu. Naopak se nezobrazují svislé čáry pro odhad úrovní. Týmy je možné řadit podle jejich názvu nebo celkového času hry.\par
\begin{figure}[H]
  \begin{center}
...
Po skončení hry, nebo i~v~jejím průběhu, je možné se přepnout do druhého pohledu jako na obr.~\ref{fig:visualizationChartFinal}. Tento pohled poskytuje celkové srovnání jednotlivých týmů. Řádky jsou zarovnány podle začátku hry. Lze je také srovnat a~řadit podle začátku libovolné úrovně. Je tak možné porovnávat celkovou délku hry nebo čas jednotlivých úrovní mezi všemi týmy.\par
...
%K celkovému rozložení na stránce byl použit grid z frameworku Bootstrap, který vizualizaci centruje a zarovnává některé její prvky. Vizualizace se pak skládá z několika částí. 
...
Přidání dalších sloupců na pravou stranu je možné. Mohly by být přidány například informace o~bodovém hodnocení, pokud budou k~dispozici odpovídající data.
...
%Vytvořená vizualizace byla začleněna do připravené kostry portletu s integrovanou Angular aplikací.
...
\end{figure}
%Kořenová komponenta obsahuje dvě komponenty \verb|GameAnalysisComponent| a \verb|ColumnHeaderComponent|, které tvoří uživatelské rozhraní.\par
...
%a komponenta vizualizace tak již s nimi může přímo pracovat.
...
%\begin{itemize}
%  \item \verb|GET /games/{gameId}| – tento zdroj podle zadaného ID hry poskytuje údaje o dané hře.\par
%  \item \verb|GET /games/{gameId}/events| –  podle specifikovaného ID hry vrací záznamy jejích událostí.\par  		
%  \item První zdroj podle zadaného ID hry poskytuje údaje o dané hře. Aplikace ho využívá k získání informace o počtu úrovní.\par
%		\verb|GET /games/{gameId}|
%  \item Další zdroj vrací podle specifikovaného ID hry záznamy jejích událostí.\par
%		\verb|GET /games/{gameId}/events|
%\end{itemize}
\begin{itemize}
  %\item \verb|GET /games/{gameId}| – tento zdroj podle zadaného ID hry poskytuje údaje o dané hře.\par
  %\item \verb|GET /games/{gameId}/events| –  podle specifikovaného ID hry vrací záznamy jejích událostí.\par  		
...
%  	\begin{itemize}
%  	  \item id hráče,
%  	  \item časové razítko události,
%  	  \item relativní čas události od začátku úrovně, udávaný v sekundách,
%  	  \item úroveň,
%  	  \item typ události.
%  	\end{itemize}
\end{itemize}
...
% NĚCO O NAHRÁNÍ SOUBORU?
%která vrací načtená data jako již popsanou datovou strukturu, se kterou může komponenta vizualizace přímo pracovat.
...
%\verb|tým, datum a čas, relativní čas, úroveň, název události|.
\begin{itemize}
  \item tým, 
...
%  \item \verb|gameColors|, \verb|lightenedColors|, \verb|planColors| – určují barvy úrovní ve vizualizaci, které se zadávají  jako pole barev v~hexadecimálním kódu, konkrétně jde o~barvy aktuálních úrovní, odehraných úrovní a~čar pro časové limity,
  \item \verb|darkColor| – tmavá barva pro ikony událostí v~aktuální úrovni,
%  \item \verb|gameColors|, \verb|planColors|, \verb|lightenedColors| – určuje jednotlivé barvy úrovní ve vizualizaci, které se zadávají  jako pole barev v hexadecimálním kódu, \verb|gameColors| značí aktuální úrovně, \verb|lightenedColors| již odehrané úrovně a \verb|planColor| barvy čar pro časové limity,
...
%dát odkaz taky na ''API'', aby bylo videt, ze to odpovida datum
%po odstraneni duplicit, seskupeni ikon atd. se povedlo neco vizualizovat

%doplnit ze se osa zjemnuje a gropovani ikon a taky Ctrl na zoom!!!!

%přepsat výpočet výšky - přibyl maxBarHeight a taky do konfigu!
...
\begin{itemize}
  ...
  \item \verb|user_events_log.csv| – CSV soubor obsahující záznamy jedné CtF hry.
\end{itemize}
