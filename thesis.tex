...
\section{Angular}
Angular je framework a platforma určená pro vývoj klientských webových aplikací v jazycích HTML a TypeScript\footnote{TypeScript je nadstavba jazyka JavaScript doplněná např. o statické typování nebo prvky z oblasti objektově-orientovaného programování, jako třídy a rozhraní. Kód je možné zkompilovat do jazyka JavaScript, který lze spustit ve webovém prohlížeči.} (případně JavaScript) vytvořený společností Google. Historicky první verzí je AngularJS psaný čistě v jazyce JavaScript. Ten byl kompletně přepsán a nahrazen verzí 2 nazvanou Angular. Postupně vznikly novější verze 4, 5 a 6, které jsou zpětně kompatibilní.\par
%Angular je platforma a MVC\footnote{MVC (Model-View-Controller) je architektonický vzor, který rozděluje software na tři nezávislé komponenty: datový model, uživatelské rozhraní a řídící logiku.} framework
%je možné zkompilovat do javascriptu nebo se kompiluje
...
\subsection{Moduly}
Základní jednotkou Angular aplikací jsou moduly (tzv. NgModule), aplikace má vždy jeden kořenový (root) modul, nazvaný podle konvence AppModule, který se stará o spuštění aplikace. Může být rozdělena do dalších funkčních modulů.\par
Modul je definován jako třída označená dekorátorem @NgModule. Dekorátor popisuje daný modul pomocí objektu s metadaty, který mu je předán jako parametr. Následující prvky tohoto objektu patří k nejdůležitějším.
...
\begin{itemize}
  \item declarations – seznam tříd uživatelského rozhraní, zejména komponent a direktiv, náležících do modulu,
  \item exports – podmnožina tříd, které mají být viditelné z ostatních modulů,
  \item imports – seznam modulů, jejichž exportované třídy jsou potřebné v tomto modulu,
  \item providers – seznam služeb (jsou zaregistrovány do globální kolekce služeb a dostupné z celé aplikace),
  \item bootstrap – hlavní kořenová komponenta (ta by měla být nastavena jen v kořenovém modulu).
\end{itemize}
